...
class RiskManager:
    ...
    def can_open_position(self, price: float, qty: float) -> bool:
        added_exposure = price * qty
        max_notional_exposure = self.equity * self.config.max_total_exposure
        return (self.current_exposure + added_exposure) <= max_notional_exposure

    def register_position_open(self, price: float, qty: float) -> None:
        self.current_exposure += price * qty

    def register_position_close(self, price: float, qty: float) -> None:
        self.current_exposure = max(0.0, self.current_exposure - (price * qty))
